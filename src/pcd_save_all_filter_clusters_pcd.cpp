...
//***************************read PCD file*****************************************
    ...
    pcl::PointCloud<pcl::PointXYZ>& point_source = *source; 
    pcl::PointCloud<pcl::PointXYZ>::Ptr pointcloud(new pcl::PointCloud<pcl::PointXYZ>());
    pcl::PointCloud<pcl::PointXYZ>& cloud = *pointcloud;
    cloud.is_dense = true;  
    //输入点云路径
    string addr = "/home/lyl/filebag/forest_1_clusters/";
    string filename;
    //待处理点云个数
    int num = 142;
    ...
    for(int i = 0;i <= num; i++)
    {
        ...
        {    
            ...
            {
                pcl::PointXYZ point;
                point.x = source->points[j].x;
                point.y = source->points[j].y;
                point.z = source->points[j].z;
    ...
    pcl::io::savePCDFileASCII("forest_1_all_clusters.pcd",cloud);
    //设置点云颜色
    pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> pointcloud_color(pointcloud,255,0,0);

    viewer->addPointCloud<pcl::PointXYZ>(pointcloud, pointcloud_color,"pointcloud",vp);

    //改变显示点云的尺寸
    viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 2, "pointcloud");
    
    
    std::cerr << "Saved " << cloud.points.size () << " data points to all_clusters.pcd." << std::endl;
